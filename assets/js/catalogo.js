...
function abrirModalProducto(producto) {
    ...
    if (producto.presentaciones.length > 0) {
        ...
        presentationSelect.append(
            `<option disabled>Sin presentaciones</option>`
        );
    }
...
function actualizarPresentacion(select) {
    ...
    $(`.product-price[data-producto="${productoId}"]`).text(
...
function actualizarPresentacionModal(select) {
    ...
    $("#detailModalPrice").text("$" + Number(precio).toLocaleString("es-CO"));
}
...
function agregarDesdeModalDescripcion() {
    ...
    if (!presentacionTexto || !precio || isNaN(cantidad) || cantidad < 1)
        return;
...
function enviarPedido() {
    ...
    mensaje += `\nTotal del pedido: $${carritoDeCompras
